...
function PoojaDetailPage() {
  ...
  const [selectedPackage, setSelectedPackage] = useState('Without Samagri')
  ...
  useEffect(() => {
    api.get(`/poojas/${id}`).then((res) => {
      setPooja(res.data)
      setSelectedPackage(res.data.packages?.[0]?.name || 'Without Samagri')
    })
  }, [id])
            ...
            {pooja.packages.map((item) => (
              ...
              >
                ...
                <p className="text-sm text-stone-600 mt-1">
                  {item.includesSamagri ? 'With Samagri' : 'Without Samagri'}
                </p>
              </button>
            ))}
          </div>
